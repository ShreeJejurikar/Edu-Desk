...
const AuthForm = ({ onLogin, onClose, showToast }) => {
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      if (isLogin) {
        const userCredential = await doSignInWithEmailAndPassword(sanitizedEmail, sanitizedPassword);
        ...
        await doCreateUserWithEmailAndPassword(sanitizedEmail, sanitizedPassword);
        ...
        try {
          ...
          // Sign out the user even if verification email fails
          await doSignOut();
          
          setError("Account created, but verification email failed to send. Please try logging in and we'll resend the verification email.");
        }
        ...
        {verificationSent && (
          <div className="mb-4 p-3 bg-green-500/20 border border-green-500/30 rounded-md" role="alert">
            <p className="text-green-300 text-sm mb-2">
              ✅ Account created successfully! Verification email sent. Please check your inbox and verify your email before logging in.
            </p>
            <button
